...
import com.zensee.android.databinding.BottomSheetGroupShareBinding

class GroupShareBottomSheet : BottomSheetDialogFragment() {
    private var _binding: BottomSheetGroupShareBinding? = null
    private val binding get() = _binding!!

    var onDestinationSelected: ((MainlandShareDestination) -> Unit)? = null
    ...
    override fun onCreateView(
        ...
        _binding = BottomSheetGroupShareBinding.inflate(inflater, container, false)
        return binding.root
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        binding.shareWechatAction.setOnClickListener {
            onDestinationSelected?.invoke(MainlandShareDestination.WECHAT)
            dismissAllowingStateLoss()
        }
        binding.shareDingTalkAction.setOnClickListener {
            onDestinationSelected?.invoke(MainlandShareDestination.DINGTALK)
            dismissAllowingStateLoss()
        }
        binding.shareMoreAction.setOnClickListener {
            onDestinationSelected?.invoke(MainlandShareDestination.MORE)
            dismissAllowingStateLoss()
        }
    ...
    companion object {
        const val TAG = "group_share_bottom_sheet"
    }
}
